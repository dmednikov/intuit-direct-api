...
try {
    $fileName = ConfigurationManager::AppSettings('VendorsFilename');
    
    $realmId = ConfigurationManager::AppSettings('RealmID'); //companyId
    $baseUrl = ConfigurationManager::AppSettings('BaseURL');

    $oauth = new OAuth( ConfigurationManager::AppSettings('ConsumerKey'), 
                        ConfigurationManager::AppSettings('ConsumerSecret'), 
                        OAUTH_SIG_METHOD_HMACSHA1, 
                        OAUTH_AUTH_TYPE_AUTHORIZATION);

    $oauth->setToken( ConfigurationManager::AppSettings('AccessToken'), 
